...
class Spatial_Aware_Layer(nn.Module):
    ...
    def forward(self, F):
        ...
        mask = torch.sigmoid(offsets_and_masks[:, 2*self.K:, : , :]) # Last 1K channels and passing it through sigmoid : original

        # Passing offsets, masks, and F into deform conv layer
        spacial_output = self.deform_conv(input=F, offset=offset,mask=mask) # original
        # spacial_output = self.deform_conv(input=F, offset=offset)
...
class DyReLUA(nn.Module):
    ...
    def forward(self, F_tensor):
        ...
        theta = self.init_values.cuda() + self.lambdas.cuda() * residuals.cuda() # Contains[alpha1(x), alpha2(x), Beta1(x), Beta2(x)]
        alphas = theta[0, :2]
        betas = theta[0, 2:]
