...
data_root = '/opt/ml/segmentation/input/mmseg_general_distill/'

# class settings
classes = ['Background', 'General trash']
palette = [
    [0, 0, 0],
    [192, 0, 128]
    ]
...
train_pipeline = [
    ...
    # dict(type='Resize', img_scale=imscale),
...
val_pipeline = [
    ...
    dict(
        ...
        transforms=[
            dict(type='Resize', img_scale=imscale, multiscale_mode='value', keep_ratio=True),
            # dict(type='Resize', img_scale=imscale, keep_ratio=True),
            # dict(
            #     type='Albu',
            #     transforms=[
            #         dict(type='CLAHE', p=1.0),
...
# optimizer = dict(type='SGD', lr=lr, weight_decay=0.01)
...
expr_name = f'UperSwinB22k_GenCLS_conv_distill_ms'
